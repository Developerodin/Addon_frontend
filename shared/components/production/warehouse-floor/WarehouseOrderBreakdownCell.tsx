...
import { collapseLinesByBrand } from "@/shared/utils/brandTransfer.util";

type Row = { article: string; kind: "in" | "out"; brand: string; qty: number };

/** Per-order warehouse receivedData (in) + transferredData (out) by article and brand. */
export default function WarehouseOrderBreakdownCell({ order }: { order: ProductionOrder }) {
  ...
  for (const a of order.articles) {
    ...
    for (const line of collapseLinesByBrand(w.receivedData)) {
      rows.push({ article: an, kind: "in", brand: line.brand ?? "—", qty: line.transferred ?? 0 });
    }
    for (const line of collapseLinesByBrand(w.transferredData)) {
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className={r.kind === "out" ? "bg-emerald-50/50" : ""}>
              ...
              <td className="border border-gray-200 px-1 py-0.5 max-w-[90px] truncate font-medium text-gray-800" title={r.brand}>
                {r.brand}
              </td>
