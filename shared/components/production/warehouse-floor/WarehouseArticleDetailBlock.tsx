...
/** Drawer detail block for Warehouse floor quantities. */
export default function WarehouseArticleDetailBlock({
  art,
  idx,
  onArticleComplete,
  completingArticleId,
  ...
  const machine =
    typeof art.machineId === "object" && art.machineId && "machineCode" in art.machineId
      ...
      : null;
  const fc = (art as { floorQuantities?: { warehouse?: Record<string, unknown> } }).floorQuantities?.warehouse as
    | {
        received?: number;
        remaining?: number;
        completed?: number;
      ...
      <table className="w-full border-collapse text-[10px]">
        <tbody>
          ...
          {fc != null && (
            <>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">WH Rcv</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{fc.received}</td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">WH Cmp</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{fc.completed ?? 0}</td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">WH Trf</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{fc.transferred ?? 0}</td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">WH Rem</td>
                <td className="border border-gray-200 px-1.5 py-0.5">
                  {fc.remaining ?? Math.max(0, (fc.received ?? 0) - (fc.transferred ?? 0))}
                </td>
              </tr>
            </>
          )}
          {order && (
            <>
              <tr>
                <td colSpan={2} className="border border-gray-200 px-1.5 py-0.5 bg-gray-100 font-bold text-gray-700">
                  Order
                </td>
              </tr>
              <tr>
