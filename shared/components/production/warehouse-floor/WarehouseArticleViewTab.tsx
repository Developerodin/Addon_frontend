...
export interface WarehouseArticleViewTabProps {
...
type WhFc = {
  ...
  transferred?: number;
  remaining?: number;
  receivedData?: Array<{ transferred?: number; styleCode?: string; brand?: string }>;
};

function getWhFc(article: Article): WhFc | undefined {
  return article.floorQuantities?.warehouse as WhFc | undefined;
}

function flattenOrdersToArticles(orders: ProductionOrder[]): WarehouseArticleRow[] {
  const rows: WarehouseArticleRow[] = [];
  for (const order of orders) {
    for (const article of order.articles) {
      const received = getWhFc(article)?.received ?? 0;
      if (received > 0) {
...
export default function WarehouseArticleViewTab({
  orders,
  onViewOrder,
  onUpdateOrder,
  getStatusBadge,
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = `warehouse-article-view-${ts}.csv`;
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No warehouse articles in current order set</p>
        </div>
      </div>
    );
  }
              ...
              return (
                ...
                >
                  <td className="pl-[10px] pr-1.5 py-2.5 border border-gray-200 align-top">
                    <WarehouseQtyStyleBrandLines lines={fc?.receivedData} variant="table" tone="received" />
                  </td>
                  <td className="px-1.5 py-2.5 border border-gray-200 align-top">
                    <div className="text-[10px] font-medium text-gray-500">{article.articleNumber ?? "—"}</div>
                    <div className="text-[9px] text-gray-400 mt-0.5">{article.linkingType ?? "N/A"}</div>
                  </td>
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200">
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ml-1 ${getPriorityBadge(order.priority)}`}
                    >
                      {order.priority}
                    </span>
                  </td>
