...
import {
  ...
  knittingM4QuantityForDisplay,
} from "./yarnEstimationFloorProgressUtils";

const fmt = (n?: number) => (n != null ? n.toLocaleString(undefined, { maximumFractionDigits: 2 }) : "—");

/** Per-article floor progress: planned qty, knitting completed, knitting M4, batch weight (from knitting). */
export const ArticleFloorStrip: React.FC<{
  ...
  const m4Qty = knittingM4QuantityForDisplay(fp);
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px]">
      ...
      <span title="floorProgress.knitting.m4Quantity">
        <span className="text-gray-500">Knitting M4</span>{" "}
        <span className="font-semibold tabular-nums text-rose-800">
          {m4Qty != null ? m4Qty.toLocaleString() : "—"}
        </span>
      </span>
...
export const OrderFloorProgressSummary: React.FC<{ orderFloorProgress: OrderFloorProgress }> = ({
  ...
  const m4Total = o.knittingM4QuantityTotal ?? 0;
  const completed = o.knittingCompletedTotal;
  const knittingFloorQtyTotal = completed + m4Total;
  return (
    <div className="px-[10px] pb-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        ...
        <div className="bg-violet-50/80 border border-violet-100 rounded p-2">
          <div className="text-[9px] font-bold text-violet-700 uppercase">Knitting floor qty</div>
          <div className="text-sm font-bold text-violet-900 tabular-nums">{knittingFloorQtyTotal.toLocaleString()}</div>
          {m4Total > 0 ? (
            <div
              className="text-[9px] text-violet-800/90 tabular-nums mt-0.5 leading-tight"
              title="Knitting completed + knitting M4 (floor)"
            >
              {completed.toLocaleString()} + {m4Total.toLocaleString()} (M4) = {knittingFloorQtyTotal.toLocaleString()}
            </div>
          ) : null}
        </div>
