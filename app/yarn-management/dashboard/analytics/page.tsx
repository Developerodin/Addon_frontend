...
function YarnAnalyticsPageContent() {
  ...
  const singleTabMode = tabParam === "purchase-orders" || tabParam === "yarn";

  const now = new Date();
  const todayStr = formatLocalYmd(now);

  const [activeTab, setActiveTab] = useState<AnalyticsTab>(
    tabParam === "yarn" ? "yarn" : "purchase-orders"
  );

  useEffect(() => {
    if (tabParam === "yarn" || tabParam === "purchase-orders") {
  ...
  return (
    <div className="main-content !p-[10px]">
      <Seo title={tabParam === "yarn" ? "Yarn Analytics" : tabParam === "purchase-orders" ? "PO Analytics" : "Yarn Analytics"} />
      ...
      <div className="bg-white shadow-sm border border-gray-100 rounded-lg overflow-hidden mx-0">
        ...
        <div className="p-[10px] border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold text-gray-800">
              {tabParam === "purchase-orders" ? "PO Analytics" : tabParam === "yarn" ? "Yarn Analytics" : "Yarn Analytics"}
            </h1>
            <HelpIcon
        ...
        {/* ─── Tab bar (hidden when navigated from sidebar with ?tab=) ─── */}
        {!singleTabMode && (
