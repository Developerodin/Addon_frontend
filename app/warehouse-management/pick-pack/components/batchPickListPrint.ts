/**
 * Printable warehouse pick list for a pick-list batch (single or combined).
 */
...
/**
 * Stock display for a batch pick line.
 * @param item - Aggregated batch item
 */
function formatBatchLineStock(item: PickListBatchItem): { text: string; noStock: boolean } {
...
/**
 * Build order chips for print header (warehouse order # + addon order ID when set).
 * @param batch - Batch detail
 */
function formatOrderChipsHtml(batch: PickListBatchDetail): string {
  ...
  if (orders.length) {
    return orders
      .map((o) => {
        ...
        const label = addon ? `${num} · Addon: ${addon}` : num;
        return `<span class="orders-chip">${escHtml(label)}</span>`;
      })
      .join("");
  }
  const fallback = (batch.orderNumbers || [])
    .map((n) => `<span class="orders-chip">${escHtml(n)}</span>`)
    .join("");
  return fallback || "—";
}

/**
 * Open a print window for a pick-list batch — warehouse staff take the paper to pick;
 * supervisor enters picked quantities back on the batch detail page.
 * @param batch - Full batch detail from the API
 */
export function printBatchPickList(batch: PickListBatchDetail) {
  const items = batch.items || [];
  const pickerLine = formatPickerLabel(batch.pickerName);
  const orderChipsHtml = formatOrderChipsHtml(batch);
  const totalRequired = items.reduce((s, i) => s + Number(i.requiredQty || 0), 0);
  const noStockCount = items.filter((i) => formatBatchLineStock(i).noStock).length;
  ...
  const itemRows = items
    .map((item, index) => {
      const stock = formatBatchLineStock(item);
...
<html><head><title>${typeLabel} – ${escHtml(batch.batchNumber)}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <h2>${escHtml(batch.batchNumber)}</h2>
  <div class="meta-block">
    ...
    ${batch.createdByName ? `<div class="meta-line"><strong>Generated by:</strong> ${escHtml(batch.createdByName)}</div>` : ""}
  </div>
  ...
  <div class="footer">
    Printed on ${new Date().toLocaleString()} · Batch ${escHtml(batch.batchNumber)}
    ${isCombined ? " · Combined orders — quantities aggregated by style code" : ""}
  </div>
</body></html>`;
