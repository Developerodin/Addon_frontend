...
import vendorM2M3M4ManagementService, {
  type VendorM4FlowRow,
  type VendorM4FlowSummary,
  type VendorM4Statistics,
...
import M4FlowDetailDrawer from "./components/M4FlowDetailDrawer";

type M4Tab = "orders" | "flow-view" | "logs";

/**
 * Vendor M4 Management — track M4 quantity and ledger for vendor production flows.
 */
export default function VendorM4ManagementPage() {
  const [activeTab, setActiveTab] = useState<M4Tab>("flow-view");
  const [rows, setRows] = useState<VendorM4FlowRow[]>([]);
  const [stats, setStats] = useState<VendorM4Statistics | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [itemsPerPage, setItemsPerPage] = useState(25);
  const [refreshKey, setRefreshKey] = useState(0);

  const [outwardRow, setOutwardRow] = useState<VendorM4FlowRow | null>(null);
  const [isOutwardSubmitting, setIsOutwardSubmitting] = useState(false);

  const [detailSummary, setDetailSummary] = useState<VendorM4FlowSummary | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [flowsRes, statsRes] = await Promise.all([
        vendorM2M3M4ManagementService.getM4Flows({ limit: 2000 }),
        vendorM2M3M4ManagementService.getM4Statistics(),
      ]);
  ...
  const handleView = async (row: VendorM4FlowRow) => {
    setShowDetail(true);
    setDetailLoading(true);
    setDetailSummary(null);
    try {
      const flowId = getVendorFlowRowId(row);
      const summary = await vendorM2M3M4ManagementService.getM4FlowSummary(flowId, 30);
  ...
  const handleOutwardSubmit = async (quantity: number, remarks: string) => {
    ...
    try {
      ...
      await vendorM2M3M4ManagementService.markM4Outward(flowId, { quantity, remarks });
  ...
  return (
    <>
      <Seo title="Vendor M4 Management" />
      <div className="main-content !p-[10px]">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-[10px]">
          <header className="flex items-center gap-2 mb-4 border-b border-gray-200 pb-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-gray-900">Vendor M4 Management</h1>
              <p className="text-[10px] text-gray-500">
                Track major defects (M4) by VPO and reference — Final Checking ledger &amp; outward
              </p>
            </div>
          </header>
          ...
          <div className="flex border-b border-gray-300 mb-3">
            ...
            <button
              type="button"
              className={`px-3 py-2 text-[11px] font-bold border-b-2 ${activeTab === "flow-view" ? "border-red-600 text-red-600" : "border-transparent text-gray-500"}`}
