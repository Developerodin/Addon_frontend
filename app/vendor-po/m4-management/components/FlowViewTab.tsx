...
/**
 * Flow-wise vendor M4 table (Final Checking only).
 */
export default function FlowViewTab({
  ...
  /** Export filtered vendor M4 flow rows as CSV. */
  const handleExportExcel = () => {
    if (filtered.length === 0) {
      toast.error("No M4 flows to export");
      return;
    }
    ...
    downloadCsv(datedExportFilename("vendor-m4-flows"), [header, ...lines]);
    ...
    <div>
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <input
          ...
          aria-label="Search vendor M4 flows"
        />
        ...
        <DownloadExcelButton
          ...
          ariaLabel="Export filtered vendor M4 flows to Excel"
          className="ml-auto"
        />
      </div>
        ...
        <table className="w-full border-collapse border border-gray-300 text-[10px] table-fixed min-w-[640px]">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="border border-gray-300 px-1 py-1 text-right bg-red-50 text-red-800 w-[90px]">
                M4 on hand
              </th>
          ...
          <tbody className="bg-white">
            {isLoading ? (
              <tr>
                <td colSpan={7} className="border border-gray-300 px-2 py-6 text-center text-gray-500">Loading…</td>
              </tr>
            ) : paged.length === 0 ? (
              <tr>
                <td colSpan={7} className="border border-gray-300 px-2 py-6 text-center text-gray-500">
                  No flows with M4 activity
                </td>
              </tr>
            ) : (
              paged.map((row) => {
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-2 text-[10px]">
          <span className="text-gray-600">Page {page} of {totalPages} ({filtered.length} rows)</span>
          <div className="flex gap-1">
            <button type="button" disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="px-2 py-1 border border-gray-300 rounded disabled:opacity-40">Prev</button>
            <button type="button" disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="px-2 py-1 border border-gray-300 rounded disabled:opacity-40">Next</button>
          </div>
        </div>
      )}
