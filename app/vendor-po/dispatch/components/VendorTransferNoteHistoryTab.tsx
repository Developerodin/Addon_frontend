...
import {
  fetchVendorDispatchTransferNotes,
  fetchVendorDispatchTransferNoteById,
  type VendorDispatchTransferNote,
...
import { downloadVendorDispatchTransferNoteExcel } from "../vendorTransferNoteExcelExport";
 ...
 */
function summarizeContainerBarcodes(note: VendorDispatchTransferNote): string {
 ...
 * @param note - Transfer note document
 */
function createdByLabel(note: VendorDispatchTransferNote): string {
...
export interface VendorTransferNoteHistoryTabProps {
  refreshKey?: number;
}

/**
 * Date-wise vendor dispatch transfer note history with re-print and Excel export.
 */
export function VendorTransferNoteHistoryTab({ refreshKey = 0 }: VendorTransferNoteHistoryTabProps) {
  const [rows, setRows] = useState<VendorDispatchTransferNote[]>([]);
  ...
  const loadHistory = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchVendorDispatchTransferNotes({
  ...
  const handleReprint = async (noteId: string) => {
    ...
    try {
      const note = await fetchVendorDispatchTransferNoteById(noteId);
  ...
  const handleExportExcel = async () => {
    setExporting(true);
    try {
      const count = await downloadVendorDispatchTransferNoteExcel(
        {
          startDate: startDate || undefined,
          endDate: endDate || undefined,
          search: search.trim() || undefined,
        },
        `vendor_dispatch_transfer_notes_${startDate || "all"}_${endDate || "all"}`
      );
      if (count === 0) {
  ...
  return (
    <div className="p-[10px] space-y-4">
      <div className="flex flex-wrap items-end gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
        <div>
          <label htmlFor="vendor-stn-history-start" className="block text-[10px] font-semibold text-gray-600 mb-1">
            From date
          </label>
          <input
            id="vendor-stn-history-start"
            type="date"
            value={startDate}
        ...
        <div>
          <label htmlFor="vendor-stn-history-end" className="block text-[10px] font-semibold text-gray-600 mb-1">
            To date
          </label>
          <input
            id="vendor-stn-history-end"
            type="date"
            value={endDate}
        ...
        <div className="flex-1 min-w-[160px]">
          <label htmlFor="vendor-stn-history-search" className="block text-[10px] font-semibold text-gray-600 mb-1">
            Search
          </label>
          <input
            id="vendor-stn-history-search"
            type="search"
            placeholder="STN serial, VPO, vendor, article…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full border border-gray-200 rounded px-2 py-1 text-[11px]"
            aria-label="Search vendor transfer notes"
          />
        </div>
        <button
          type="button"
          ...
          aria-label="Download vendor transfer note report as Excel"
        >
          {exporting ? (
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-3 py-2 bg-white border-b border-gray-100 flex items-center justify-between">
          <span className="text-[11px] font-bold text-gray-700">Vendor transfer note history</span>
