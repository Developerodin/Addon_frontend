...
import { mapActiveItemToDisplay } from "../utils/vendorContainerDisplay";
...
function flattenContainerLines(containers: ContainerMaster[]): UpcomingLineRow[] {
  ...
  for (const container of containers) {
    for (const item of getVendorActiveItems(container)) {
      const display = mapActiveItemToDisplay(item);
      rows.push({
        container,
        qty: display.qty,
        vpoNumber: display.vpoNumber,
        vendorCode: display.vendorCode,
        productLabel: display.productName,
        refCode: display.referenceCode,
      });
    }
  }
      ...
      <div className="p-[10px] flex flex-wrap items-center justify-between gap-3 border-b border-gray-100">
        <div className="relative w-full sm:w-72 min-w-[180px]">
          <input
            ...
            placeholder="Search container, barcode, VPO, vendor code..."
            value={searchQuery}
            ...
            <tbody>
              {filteredRows.map((row, idx) => (
                <tr key={rowKey(row, idx)} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-1.5 py-2 border border-gray-200 text-[10px] font-mono font-semibold text-gray-800">
                    {row.vendorCode}
                  </td>
                      ...
                      <tbody>
                        {group.rows.map((row, idx) => (
                          <tr key={rowKey(row, idx)} className="hover:bg-gray-50/50">
                            ...
                            <td className="px-1.5 py-1.5 border border-gray-200 text-[10px] font-mono font-semibold text-gray-800">
                              {row.vendorCode}
                            </td>
