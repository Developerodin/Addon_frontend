...
export default function LogsTab({ refreshKey = 0 }: LogsTabProps) {
  const [logs, setLogs] = useState<VendorM3LogEntry[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [search, setSearch] = useState("");
  const [type, setType] = useState<(typeof TYPE_OPTIONS)[number]>("");
  const [sourceFloor, setSourceFloor] = useState<(typeof FLOOR_OPTIONS)[number]>("");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        const res = await vendorM2M3M4ManagementService.getM3Logs({
          page,
          limit,
  ...
  /** Export all filtered vendor M3 logs as CSV. */
  const handleExportExcel = async () => {
    setIsExporting(true);
    try {
      const rows = await fetchAllPaginatedResults<VendorM3LogEntry>((pageNum, pageLimit) =>
        vendorM2M3M4ManagementService.getM3Logs({
          page: pageNum,
          limit: pageLimit,
          search: search.trim() || undefined,
          type: type || undefined,
      ...
      downloadCsv(datedExportFilename("vendor-m3-logs"), [header, ...lines]);
    ...
    <div>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 mb-3">
        ...
        <select
          value={sourceFloor}
          onChange={(e) => {
            setSourceFloor(e.target.value as (typeof FLOOR_OPTIONS)[number]);
            setPage(1);
          }}
          className="py-1.5 px-2 text-[11px] border border-gray-300 rounded"
        ...
        >
          ...
          <option value="secondaryChecking">Secondary Checking</option>
          <option value="finalChecking">Final Checking</option>
        </select>
        ...
        <div className="col-span-2 md:col-span-4 lg:col-span-6 flex justify-end">
          <DownloadExcelButton
            ...
            ariaLabel="Export filtered vendor M3 logs to Excel"
          />
        </div>
      </div>
