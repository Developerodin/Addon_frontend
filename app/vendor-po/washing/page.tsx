...
import vendorProductionFlowService, { VendorProductionFlow, type WashingFloorPatchPayload } from "@/shared/services/vendorProductionFlowService";
...
const WashingPage = () => {
  ...
  const loadFlows = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to load washing flows");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleOpenProcess = (flow: VendorProductionFlow) => {
    ...
    setCompletedQty(flow.floorQuantities.washing.completed ?? 0);
  ...
  const handleSaveProcessing = async () => {
    ...
    setSaving(true);
    try {
      const payload: WashingFloorPatchPayload = { completed: Number(completedQty) || 0 };
      await vendorProductionFlowService.updateFloor(selectedFlow.id, "washing", payload);
      toast.success("Washing floor updated");
  ...
  const getStatusBadge = (flow: VendorProductionFlow) => {
    const isCompleted = flow.floorQuantities.washing.completed > 0;
  ...
  return (
    <div className="main-content !p-[10px]">
        ...
        <div className="p-[10px]">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-sm font-bold text-gray-800">Washing Stage</h1>
              <HelpIcon
                title="Washing Process"
                content="Items that have passed secondary checking move here for washing and related treatments."
              />
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <table className="w-full border-collapse border border-gray-200">
              ...
              <tbody>
                {paginatedFlows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-1.5 py-10 text-center text-gray-400 text-xs font-bold uppercase tracking-widest border border-gray-200">
                      No washing batches found
                    </td>
                  </tr>
                ) : (
                  paginatedFlows.map((flow) => {
                    const ws = flow.floorQuantities.washing;
                    ...
                    return (
                      <tr key={flow.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-1.5 py-2.5 text-right font-medium text-[12px] text-gray-700 border border-gray-200">{ws.received.toLocaleString()}</td>
                        <td className="px-1.5 py-2.5 text-right font-bold text-[12px] text-emerald-600 border border-gray-200">{ws.completed.toLocaleString()}</td>
                        <td className="px-1.5 py-2.5 text-right text-[12px] text-sky-600 border border-gray-200">{ws.transferred.toLocaleString()}</td>
                        <td className="px-1.5 py-2.5 border border-gray-200">
                          <span className={getStatusBadge(flow)}>{ws.completed > 0 ? "Processed" : "Pending"}</span>
                        </td>
                        <td className="px-1.5 py-2.5 border border-gray-200">
                          <div className="flex items-center gap-2">
                            <button
      ...
      <VendorProductionFloorDrawer
        ...
        title={`Washing — ${selectedFlow?.referenceCode || selectedFlow?.id.slice(-6) || ""}`}
        titleId="vendor-washing-drawer-title"
        ...
        hint={
          <p className={CRM.drawerHint}>
            <strong>Washing:</strong> only <strong>completed</strong> is sent to the API (no received or transferred in this
            request).
          </p>
        }
      >
