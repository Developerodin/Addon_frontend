"use client";

import React from "react";
import Seo from "@/shared/layout-components/seo/seo";
import Link from "next/link";
import { useNavigation } from "@/shared/contextapi/navigationContext";

const purchaseManagementModules = [
  {
    title: "Vendor List",
    description: "Manage vendors and catalog-linked products",
    icon: "ri-user-search-line",
    path: "/vendor-po/vendor-list",
    permission: "Vendor List",
  },
  {
    title: "Purchase Order",
    description: "Create and manage vendor purchase orders",
    icon: "ri-shopping-cart-line",
    path: "/vendor-po/purchase-management/purchase",
    permission: "Vendor PO Raise",
  },
  {
    title: "Purchase Order Received",
    description: "Track goods inward and receive against POs",
    icon: "ri-inbox-line",
    path: "/vendor-po/purchase-management/purchase-order-received",
    permission: "Vendor PO Receive",
  },
];

export default function VendorPOPurchaseManagementPage() {
  const { hasSubPermission, isLoading } = useNavigation();

  if (isLoading) {
    return (
      <div className="main-content">
        <div className="flex justify-center items-center py-12">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4" />
            <p className="text-gray-600">Loading permissions...</p>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="main-content">
      <Seo title="Purchase Management" />

      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-12">
          <div className="box !bg-transparent border-0 shadow-none">
            <div className="box-header">
              <h1 className="box-title text-2xl font-semibold">Purchase Management</h1>
              <p className="text-gray-600 mt-2">
                Vendor procurement: vendors, purchase orders, and goods receipt — aligned with yarn Purchase Management.
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {purchaseManagementModules.map((module) => {
              const hasPermission = hasSubPermission("/vendor-po", module.permission);

              return (
                <div
                  key={module.title}
                  className="box group hover:shadow-lg transition-shadow duration-300"
                >
                  <div className="box-body text-center">
                    <div className="mb-4">
                      <div className="inline-flex items-center justify-center w-16 h-16 bg-primary/10 rounded-full text-primary text-2xl group-hover:bg-primary group-hover:text-white transition-colors duration-300">
                        <i className={module.icon} />
                      </div>
                    </div>

                    <h3 className="text-lg font-semibold text-gray-900 mb-2">{module.title}</h3>

                    <p className="text-gray-600 text-sm mb-4">{module.description}</p>

                    {hasPermission ? (
                      <Link href={module.path} className="ti-btn ti-btn-primary ti-btn-sm w-full">
                        Access Module
                        <i className="ri-arrow-right-line ms-2" />
                      </Link>
                    ) : (
                      <div className="ti-btn ti-btn-light ti-btn-sm w-full cursor-not-allowed opacity-50">
                        Access Restricted
                        <i className="ri-lock-line ms-2" />
                      </div>
                    )}
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </div>
  );
}
