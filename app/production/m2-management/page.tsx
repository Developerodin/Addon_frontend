...
/**
 * M2 Management — track and resolve M2 repair entries from QC floors.
 */
export default function M2ManagementPage() {
  const [activeTab, setActiveTab] = useState<M2Tab>("entries");
  const [stats, setStats] = useState<M2Statistics | null>(null);
  const [isLoadingStats, setIsLoadingStats] = useState(false);
  const [refreshKey, setRefreshKey] = useState(0);
  const [resolveEntry, setResolveEntry] = useState<M2EntryRow | null>(null);
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const statsRes = await productionService.getM2Statistics();
  ...
  const openResolve = (entry: M2EntryRow, action: M2ResolveAction) => {
    setResolveEntry(entry);
    setResolveAction(action);
  };

  const handleResolveSubmit = async (quantity: number, remarks: string, transferItems?: TransferItem[]) => {
    if (!resolveEntry) return;
    setIsSubmitting(true);
    try {
      const body = transferItems?.length
        ? { quantity, remarks, transferItems }
        : { quantity, remarks };
      let res;
      if (resolveAction === "merge") {
        res = await productionService.mergeM2ToM1(resolveEntry.entryId, body);
      } else if (resolveAction === "m3") {
        res = await productionService.transferM2ToM3(resolveEntry.entryId, body);
      } else {
        res = await productionService.transferM2ToM4(resolveEntry.entryId, body);
      }
      if (!res.success) {
      ...
      <div className="main-content !p-[10px]">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-[10px]">
          <header className="flex items-center gap-2 mb-4 border-b border-gray-200 pb-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-gray-900">M2 Management</h1>
              <p className="text-[10px] text-gray-500">
                Resolve repairable M2 from Checking / Secondary / Final — merge cascades to all downstream floors
              </p>
            </div>
          </header>
