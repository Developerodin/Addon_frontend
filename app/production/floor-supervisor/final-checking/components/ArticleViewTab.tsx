...
/**
 * M2/M3/M4 defect quantities for final checking (floor snapshot, then article-level fallback).
 */
function finalCheckingDefectQuantities(article: Article): {
  m2: number;
  m3: number;
  m4: number;
} {
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => finalCheckingRemaining(r.article) > 0);
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = `final-checking-article-view-${ts}.csv`;
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No final checking articles on this floor</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-[10px]">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        >
          <i className="ri-file-excel-2-line text-xs text-emerald-600" /> Download Excel
        </button>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200 text-[10px] text-gray-600 max-w-[140px]">
                    {(fc?.receivedData as Array<{ transferred?: number; styleCode?: string; brand?: string }> | undefined)?.some((d) => (d.transferred ?? 0) > 0) ? (
                      <div className="space-y-0.5">
                        {(fc?.receivedData as Array<{ transferred?: number; styleCode?: string; brand?: string }>)
                          .filter((d) => (d.transferred ?? 0) > 0)
                          .slice(0, 3)
                          .map((d, i) => (
                        ...
                        {(fc?.receivedData as Array<{ transferred?: number }>).filter((d) => (d.transferred ?? 0) > 0).length > 3 && (
                          <div className="text-gray-400">
                            +{(fc?.receivedData as Array<{ transferred?: number }>).filter((d) => (d.transferred ?? 0) > 0).length - 3} more
                          </div>
                        )}
                      </div>
                    ) : (
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200 text-[10px] text-gray-600 max-w-[140px]">
                    {fc?.transferredData?.length ? (
                      <div className="space-y-0.5">
                        {fc.transferredData.slice(0, 3).map((d, i) => (
                          <div key={i} className="truncate">
                            {d.transferred ?? 0} {d.styleCode ? `· ${d.styleCode}` : ""} {d.brand ? `· ${d.brand}` : ""}
                          </div>
                        ))}
                        {fc.transferredData.length > 3 && (
                          <div className="text-gray-400">+{fc.transferredData.length - 3} more</div>
                        )}
                      </div>
                    ) : (
                      "—"
                  ...
                  <td className="px-1.5 py-2.5 text-right pr-[10px] border border-gray-200">
                    <div className="flex items-center justify-end gap-1 opacity-80 group-hover:opacity-100 flex-wrap">
                      ...
                      <button
                        ...
                        title="Update order"
                        aria-label={`Update order ${orderLabel}`}
                      >
      ...
      {filteredRows.length === 0 && !articleSearch.trim() && orders.length > 0 && (
        <div className="py-8 text-center text-[11px] text-gray-500">
          {showAllArticles
            ? "No final checking lines to display."
            ...
            {Array.from({ length: Math.min(articleTotalPages, 7) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-7 h-7 flex items-center justify-center text-[11px] font-bold rounded ${safeArticlePage === pageNum ? "bg-teal-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-50"}`}
