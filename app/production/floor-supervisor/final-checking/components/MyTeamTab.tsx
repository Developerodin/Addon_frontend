...
const FLOOR_FINAL_CHECKING = "Final Checking" as const;

/**
 * Shows all team members whose workingFloor is Final Checking.
 */
  ...
  const fetchTeam = useCallback(async () => {
    try {
      ...
      const data = await teamMasterService.list({
        workingFloor: FLOOR_FINAL_CHECKING,
        page: 1,
        limit: 200,
    ...
    <div className="p-[10px]">
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <span className="text-[11px] font-medium text-[#495057]">
          {totalResults} member{totalResults !== 1 ? "s" : ""} on Final Checking floor
        </span>
      </div>
            ...
            {members.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-[11px] text-gray-500">
                  No team members found for Final Checking floor.
                </td>
              </tr>
            ) : (
              members.map((row) => {
                    ...
                    <td className="px-1.5 py-2.5 border border-gray-200">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${
                          row.role === "Supervisor" ? "bg-teal-50 text-teal-700 border border-teal-100" : "bg-gray-50 text-gray-600 border border-gray-200"
                        }`}
                      >
                        {row.role}
                      </span>
