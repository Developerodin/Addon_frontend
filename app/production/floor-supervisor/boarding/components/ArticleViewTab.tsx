...
/** Boarding remaining qty (prefers API remaining, else received − transferred). */
function boardingRemaining(article: Article): number {
  const boarding = article.floorQuantities?.boarding;
...
/** Flattens orders into rows with boarding received > 0. */
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => boardingRemaining(r.article) > 0);
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredRows.map(({ article, order }) => {
      ...
      const board = article.floorQuantities?.boarding;
    ...
    a.href = url;
    a.download = `boarding-article-view-${ts}.csv`;
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No boarding articles on this floor</p>
        </div>
      </div>
    );
  }
