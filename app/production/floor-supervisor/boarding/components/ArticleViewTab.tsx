...
export default function ArticleViewTab({
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredRows.map(({ article, order }) => {
      ...
      const board = (article as { floorQuantities?: { boarding?: { received?: number; completed?: number; transferred?: number; remaining?: number } } }).floorQuantities?.boarding;
      const received = board?.received ?? 0;
      const done = board?.completed ?? 0;
