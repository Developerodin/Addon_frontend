...
const BoardingFloorSupervisorPage = () => {
  ...
  const [updateData, setUpdateData] = useState<{[key: string]: {boardedQuantity: number}}>({});
  ...
  const handleUpdateOrder = (order: ProductionOrder) => {
    ...
    const initialData: {[key: string]: {boardedQuantity: number}} = {};
    order.articles.forEach(article => {
      initialData[article.id] = {
        boardedQuantity: article.completedQuantity
      };
    });
      ...
      order.id === selectedOrder.id 
        ? {
            ...
            articles: order.articles.map(article => {
              ...
              if (update) {
                const newProgress = Math.round((update.boardedQuantity / article.plannedQuantity) * 100);
                const newStatus = update.boardedQuantity >= article.plannedQuantity ? 'Completed' : 
                                 update.boardedQuantity > 0 ? 'In Progress' : 'Pending';
                // Log transfers from Boarding to Branding when boarded increases
                const previousCompleted = article.completedQuantity || 0;
                const deltaTransferred = Math.max(0, update.boardedQuantity - previousCompleted);
                ...
                if (deltaTransferred > 0) {
                  ...
                  newLogs.push({
                    ...
                    date: isoDate,
                    action: 'Transferred to Branding',
                ...
                return {
                  ...article,
                  completedQuantity: update.boardedQuantity,
          ...
          <div className="box !bg-transparent border-0 shadow-none">
            <div className="box-header flex justify-between items-center">
              <div className="flex items-center space-x-3">
                <h1 className="box-title text-2xl font-semibold">Boarding Supervisor Dashboard</h1>
                <HelpIcon
                  title="Boarding Supervisor Dashboard"
                  content={
                    <div className="space-y-4">
                      <div>
                        <h4 className="font-semibold text-lg mb-2">What is this page?</h4>
                        <p className="text-gray-700">
                          Boarding (by Boarding Supervisor): Enter article-wise boarding quantities.
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <ul className="list-disc list-inside space-y-1 text-gray-700">
                          <li><strong>Enter Boarded Quantity:</strong> Update boarded quantity for each article</li>
                          <li><strong>See Received:</strong> View quantity received from Washing</li>
                          <li><strong>Auto Move:</strong> On update, article moves to Branding</li>
      ...
      {showUpdateModal && selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            {/* Articles Update Form (Tabs + Quantity from Washing) */}
                ...
                return (
                  <div className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      ...
                      <div>
                        <label className="form-label">Quantity from Washing</label>
                        <div className="text-lg font-semibold text-blue-600">{article.quantityFromWashing.toLocaleString()}</div>
                      </div>
                      <div>
                        <label className="form-label">Boarded Quantity *</label>
                        <input
                          type="number"
                          className="form-control"
                          value={updateData[article.id]?.boardedQuantity || 0}
                          onChange={(e) => handleQuantityChange(article.id, Number(e.target.value))}
                          min="0"
                          max={article.quantityFromWashing}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-600">
                      <div>
                        Remaining: {(article.quantityFromWashing - (updateData[article.id]?.boardedQuantity || 0)).toLocaleString()}
                      </div>
                      <div>
                        Progress: {Math.round(((updateData[article.id]?.boardedQuantity || 0) / article.plannedQuantity) * 100)}%
                      </div>
                    </div>
                        ...
                        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded max-w-full overflow-x-auto">
                          <ul className="list-disc list-inside text-sm text-blue-900 space-y-1 break-words">
                            {(
                              ...
                              <li key={log.id}>
                                {(log.fromFloor || 'Boarding')} {log.action?.toLowerCase?.() || 'action'} {log.quantity.toLocaleString()} {log.toFloor ? `to ${log.toFloor}` : ''} on {log.date}
