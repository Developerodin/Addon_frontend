...
/** Flattens orders into rows; parent already applied dispatch article filters. */
...
export default function ArticleViewTab({
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredRows.map(({ article, order }) => {
      const fc = (article as any).floorQuantities?.dispatch;
      ...
      const receivedData = ((fc?.receivedData as any[]) ?? [])
        .filter((d: any) => (d.transferred ?? 0) > 0)
        .map((d: any) => `${d.transferred ?? 0}${d.styleCode ? ` · ${d.styleCode}` : ""}${d.brand ? ` · ${d.brand}` : ""}`)
        .join(" | ") || "—";
      const transferredData = ((fc?.transferredData as any[]) ?? [])
        .map((d: any) => `${d.transferred ?? 0}${d.styleCode ? ` · ${d.styleCode}` : ""}${d.brand ? ` · ${d.brand}` : ""}`)
        .join(" | ") || "—";
    ...
    a.href = url;
    a.download = `dispatch-article-view-${ts}.csv`;
        ...
        <table className="w-full border-collapse border border-gray-200">
          ...
          <tbody>
            {filteredRows.map(({ article, order }) => {
              ...
              const completed = fc?.m1Quantity ?? (article as any).m1Quantity ?? 0;
              const transferred = fc?.transferred ?? 0;
              const remaining = fc?.remaining ?? Math.max(0, received - transferred); // backend sends remaining; fallback for legacy
              ...
              return (
                ...
                >
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200 text-[10px] text-gray-600 max-w-[140px]">
                    {(fc?.receivedData as any[])?.some((d: any) => (d.transferred ?? 0) > 0) ? (
                      <div className="space-y-0.5">
                        {(fc?.receivedData as any[]).filter((d: any) => (d.transferred ?? 0) > 0).slice(0, 3).map((d: any, i: number) => (
                          <div key={i} className="truncate">
                            {d.transferred ?? 0} {d.styleCode ? `· ${d.styleCode}` : ""} {d.brand ? `· ${d.brand}` : ""}
                          </div>
                        ))}
                        {(fc?.receivedData as any[])?.filter((d: any) => (d.transferred ?? 0) > 0).length > 3 && (
                          <div className="text-gray-400">+{(fc?.receivedData as any[]).filter((d: any) => (d.transferred ?? 0) > 0).length - 3} more</div>
                        )}
                      </div>
                    ) : (
                      "—"
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200 text-[10px] text-gray-600 max-w-[140px]">
                    {fc?.transferredData?.length > 0 ? (
                      <div className="space-y-0.5">
                        {fc.transferredData.slice(0, 3).map((d: any, i: number) => (
                          <div key={i} className="truncate">
                            {d.transferred ?? 0} {d.styleCode ? `· ${d.styleCode}` : ""} {d.brand ? `· ${d.brand}` : ""}
                          </div>
                        ))}
