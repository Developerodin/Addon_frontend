...
function dispatchRemaining(article: Article): number {
  const d = article.floorQuantities?.dispatch;
  if (d == null) return 0;
...
/** Flattens orders into rows with dispatch received > 0. */
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => dispatchRemaining(r.article) > 0);
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No dispatch articles on this floor</p>
        </div>
      </div>
    );
  }
