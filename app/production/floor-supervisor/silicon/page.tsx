...
const SiliconFloorSupervisorPage = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if (response.success) {
        console.log('Silicon orders loaded:', response.data.results);
        setOrders(response.data.results);
        setTotalPages(response.data.totalPages);
        setTotalResults(response.data.totalResults);
      } else {
        console.error('Failed to load silicon orders:', response.error);
        toast.error('Failed to load silicon orders');
      }
    } catch (error: any) {
      console.error('Error loading silicon orders:', error);
      toast.error(error.message || 'Failed to load silicon orders');
    } finally {
      setIsLoading(false);
    }
  };
