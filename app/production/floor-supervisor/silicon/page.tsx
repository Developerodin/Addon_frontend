...
type SiliconTab = "orders" | "article-view" | "my-team" | "upcoming";

const FLOOR_CATALOG_LIMIT = 2000;

const SiliconFloorSupervisorPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<SiliconTab>("article-view");
  ...
  /** Loads silicon floor orders for both tabs; filter + paginate client-side. */
  const loadFloorOrdersCatalog = useCallback(async () => {
    ...
    try {
      ...
      const response = await productionService.getFloorOrders("Silicon", apiFilters);

      if (response.success) {
        setFloorCatalog(response.data.results);
      } else {
        console.error("Failed to load silicon floor orders:", response.error);
        toast.error("Failed to load silicon orders");
      }
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Failed to load silicon orders";
      console.error("Error loading silicon floor orders:", error);
      toast.error(msg);
    } finally {
  ...
  const handleAcceptArticleQuantity = async () => {
    ...
    try {
      ...
      toast.success("Container accepted on Silicon.");
      setShowContainerScanDrawer(false);
      setContainerScanned(null);
      setContainerScanBarcode("");
      void loadFloorOrdersCatalog();
  ...
  const handleOpenAssignDrawer = useCallback(async () => {
    ...
    try {
      const data = await teamMasterService.list({ workingFloor: "Silicon", limit: 200 });
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      const updatePromises = selectedOrder.articles.map(async (article) => {
        ...
        const update = updateData[articleId];
        const siliconTransferredQuantity = article.floorQuantities?.silicon?.transferred || 0;
        if (update && (update.completedQuantity !== siliconTransferredQuantity || update.remarks !== (article.remarks || ''))) {
          const progressData = {
  ...
  return (
    <div className="main-content !p-[10px]">
      <Seo title="Silicon Supervisor Dashboard"/>
        ...
        <div className="p-[10px]">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-sm font-bold text-gray-800">Silicon Floor Supervisor</h1>
              <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm">
                {filteredOrders.length}
              </span>
              <HelpIcon
                title="Silicon Floor Supervisor Dashboard"
                content={
                  <div className="space-y-4">
                    <div>
                      <h4 className="font-semibold text-lg mb-2">What is this page?</h4>
                      <p className="text-gray-700">
                        This is the Silicon Floor Supervisor Dashboard where you can view and update production orders on the Silicon floor.
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-lg mb-2">What can you do here?</h4>
                      <ul className="list-disc list-inside space-y-1 text-gray-700">
                        <li><strong>View Orders:</strong> See all orders with articles on the Silicon floor</li>
                        ...
                        <li><strong>Update Progress:</strong> Enter completed quantity, scan container, then submit and transfer to next floor</li>
          ...
          {catalogLoading && floorCatalog.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-[10px] text-gray-500">
                {hasActiveFilters ? 'Try adjusting filters or search' : 'No orders on Silicon floor'}
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-[10px] text-gray-500">Turn on Show all to include orders with zero remaining on Silicon</p>
            </div>
          ) : (
                  ...
                  {paginatedOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-1.5 py-2.5 border border-gray-200">
                        ...
                        {order.articles.some(a => (a as any).floorQuantities?.silicon) && (
                          <div className="text-[10px] text-purple-600 mt-0.5">
                            R:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.silicon?.received || 0), 0)} Trf:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.silicon?.transferred || 0), 0)} Rem:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.silicon?.remaining ?? 0), 0)}
                          </div>
                        )}
                      </td>
                      <td className="px-1.5 py-2.5 border border-gray-200">
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-4xl bg-white shadow-xl z-50 flex flex-col overflow-hidden border-l border-gray-200">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  onClick={() => {
                    ...
                    if (!firstWithQty) { toast.error("Enter at least one article with silicon completed quantity"); return; }
                    setUpdateContainerBarcode("");
                    setUpdateContainerCheckStatus("idle");
                    setUpdateContainerFetched(null);
            ...
            <div className="mb-4 px-3 py-2 rounded-md bg-purple-50 border border-purple-200 text-[11px] text-gray-800">
              <strong>How to update:</strong> Enter silicon completed quantity and remarks per article. Then click Update Order, scan the bag/container, select article and next floor, and submit.
            </div>
                ...
                <table className="min-w-full text-xs border-collapse">
                  <thead className="bg-gray-100 border-b border-gray-300">
                    <tr>
                      ...
                      <th className="px-2 py-1.5 text-center font-semibold text-gray-700 border-r border-gray-300 whitespace-nowrap bg-purple-50">Silicon Completed *</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {modalArticles.map((article, idx) => {
                      ...
                      const transferredQty = article.floorQuantities?.silicon?.transferred || 0;
                      ...
                      return (
                        <tr key={articleId} className="hover:bg-gray-50">
                          ...
                          <td className="px-2 py-1.5 border-r border-gray-300 align-top min-w-[80px]">
                            <ReceivedQuantityDisplay received={receivedQty} repairReceived={article.floorQuantities?.silicon?.repairReceived} repairFromFloor={article.floorQuantities?.silicon?.repairFromFloor} />
                          </td>
                          ...
                          <td className="px-2 py-1.5 border-r border-gray-300 bg-purple-50">
                            <div className="flex flex-col gap-0.5">
                              <NumericInput
                                ...
                                placeholder={isFullyTransferred ? 'Done' : `Max ${remainingQty}`}
                          ...
                          <td className="px-2 py-1.5">
                            <textarea className="w-full py-1 px-2 text-[11px] border border-gray-300 rounded resize-none" rows={1} placeholder="Remarks..." value={currentUpdateData.remarks} onChange={(e) => handleRemarksChange(articleId, e.target.value)} />
                          </td>
                        </tr>
                      );
                    })}
      ...
      {showUpdateContainerModal && selectedOrder && (() => {
        ...
        const nextFloor = firstWithQty ? (updateContainerNextFloor || "Boarding") : "";
        return (
        <>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl border border-gray-200 max-w-md w-full p-4 flex flex-col">
              ...
              <p className="text-[11px] text-gray-600 mb-3">Scan container for silicon completed articles. All articles with completed qty will be added.</p>
              <div className="space-y-2 mb-3">
                <label className="block text-[10px] font-bold text-gray-600 mb-0.5">Container barcode</label>
                <input type="text" className="w-full border border-gray-200 rounded px-3 py-2 text-[11px] focus:ring-0 focus:border-purple-300 placeholder:text-gray-400 font-medium" placeholder="Scan or enter barcode..." value={updateContainerBarcode} onChange={(e) => setUpdateContainerBarcode(e.target.value)} />
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-gray-200">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const newRows = articlesWithQty.map(({ article, quantity }) => ({ article: article._id ?? article.id ?? "", quantity })).filter((i) => i.article);
                ...
                >
                  {updateContainerSubmitting ? "..." : "Update & submit order"}
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  {containerScanned.articles.length > 0 ? (
                    <button type="button" onClick={handleAcceptArticleQuantity} disabled={acceptArticleLoading || !containerBelongsToCurrentFloor} className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white text-[11px] font-bold rounded hover:bg-emerald-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                      {acceptArticleLoading ? "..." : "Accept container (Silicon)"}
                    </button>
                  ) : (
      ...
      {showAssignDrawer && (
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-xl z-50 flex flex-col overflow-hidden border-l border-gray-200">
            <div className="flex justify-between items-center p-[10px] border-b border-gray-200 flex-shrink-0">
              <h3 className="text-sm font-bold text-gray-800">Assign to team member (Silicon)</h3>
                  ...
                  <table className="min-w-full text-xs border-collapse">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {modalArticles.map((article, idx) => {
                        ...
                        const transferredQty = article.floorQuantities?.silicon?.transferred || 0;
                        const remainingQty = article.floorQuantities?.silicon?.remaining || 0;
...
export default SiliconFloorSupervisorPage;
