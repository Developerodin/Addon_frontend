...
/** Silicon remaining qty (prefers API remaining, else received − transferred). */
function siliconRemaining(article: Article): number {
  const silicon = article.floorQuantities?.silicon;
  if (silicon == null) return 0;
  if (typeof silicon.remaining === "number") return silicon.remaining;
  const received = silicon.received ?? 0;
  const transferred = silicon.transferred ?? 0;
  return Math.max(0, received - transferred);
}

/** Flattens orders into rows with silicon received > 0. */
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => siliconRemaining(r.article) > 0);
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No silicon articles on this floor</p>
        </div>
      </div>
    );
  }
