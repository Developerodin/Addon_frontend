...
const WashingFloorSupervisorPage = () => {
  ...
  // Tabs + container scan + assign + update-container flow (like checking)
  const [activeTab, setActiveTab] = useState<WashingTab>("article-view");
  ...
  const loadFloorOrdersCatalog = useCallback(async () => {
    ...
    try {
      ...
      const response = await productionService.getFloorOrders("Washing", apiFilters);
      if (response.success) {
        setFloorCatalog(response.data.results);
      } else {
        console.error("Failed to load washing floor orders:", response.error);
        toast.error("Failed to load washing orders");
      }
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Failed to load washing orders";
      console.error("Error loading washing floor orders:", error);
      toast.error(msg);
    } finally {
  ...
  const filterOrdersByReceivedQuantity = (orders: ProductionOrder[], showAll: boolean): ProductionOrder[] => {
    return orders.map(order => {
      const filteredArticles = order.articles.filter(article => {
        ...
        const remaining = article.floorQuantities?.washing?.remaining ?? (received - transferred);
  ...
  const getWashingFloorData = (article: Article) => ({
    floor: "washing" as const,
    data: article.floorQuantities?.washing,
  });
  ...
  const handleAcceptArticleQuantity = async () => {
    ...
    try {
      ...
      toast.success("Container accepted on Washing.");
      setShowContainerScanDrawer(false);
      setContainerScanned(null);
      setContainerScanBarcode("");
      void loadFloorOrdersCatalog();
  ...
  const handleOpenAssignDrawer = useCallback(async () => {
    ...
    try {
      const data = await teamMasterService.list({ workingFloor: "Washing", limit: 200 });
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      const updatePromises = selectedOrder.articles.map(async (article) => {
        ...
        const update = updateData[articleId];
        const washingTransferredQuantity = article.floorQuantities?.washing?.transferred || 0;
        if (update && (update.completedQuantity !== washingTransferredQuantity || update.remarks !== (article.remarks || ''))) {
          const progressData = {
        ...
        <div className="p-[10px]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <HelpIcon
                title="Washing Floor Supervisor Dashboard"
                content={
                  <div className="space-y-4">
                    <div>
                      <h4 className="font-semibold text-lg mb-2">What is this page?</h4>
                      <p className="text-gray-700">
                        This is the Washing Floor Supervisor Dashboard where you can view and update production orders that are currently on the Washing floor.
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-lg mb-2">What can you do here?</h4>
                      <ul className="list-disc list-inside space-y-1 text-gray-700">
                        <li><strong>View Orders:</strong> See all orders with articles on the Washing floor</li>
          ...
          {catalogLoading && floorCatalog.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-[10px] text-gray-500">
                {hasActiveFilters ? 'Try adjusting filters or search' : 'No orders on Washing floor'}
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-[10px] text-gray-500">Turn on Show all to include orders with zero remaining on Washing</p>
            </div>
          ) : (
                ...
                <table className="min-w-full text-xs border-collapse">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {modalArticles.map((article, idx) => {
                      ...
                      const receivedQty = article.floorQuantities?.washing?.received || 0;
                      const transferredQty = article.floorQuantities?.washing?.transferred || 0;
                      ...
                      return (
                        <tr key={articleId} className="hover:bg-gray-50">
                          ...
                          <td className="px-2 py-1.5 border-r border-gray-300 align-top min-w-[80px]">
                            <ReceivedQuantityDisplay received={receivedQty} repairReceived={article.floorQuantities?.washing?.repairReceived} repairFromFloor={article.floorQuantities?.washing?.repairFromFloor} />
                          </td>
        ...
        <>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl border border-gray-200 max-w-md w-full p-4 flex flex-col">
              ...
              <p className="text-[11px] text-gray-600 mb-3">Scan container for washing completed articles. All articles with completed qty will be added.</p>
                ...
                <div className="space-y-4">
                  ...
                  {containerScanned.articles.length > 0 ? (
                    <button type="button" onClick={handleAcceptArticleQuantity} disabled={acceptArticleLoading || !containerBelongsToCurrentFloor} className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white text-[11px] font-bold rounded hover:bg-emerald-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                      {acceptArticleLoading ? "..." : "Accept container (Washing)"}
                    </button>
                  ) : (
                  ...
                  <table className="min-w-full text-xs border-collapse">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {modalArticles.map((article, idx) => {
                        const plannedQty = article.plannedQuantity || 0;
                        const receivedQty = article.floorQuantities?.washing?.received || 0;
                        const completedQty = article.floorQuantities?.washing?.completed || 0;
                        const transferredQty = article.floorQuantities?.washing?.transferred || 0;
                        const remainingQty = article.floorQuantities?.washing?.remaining || 0;
...
export default WashingFloorSupervisorPage;
