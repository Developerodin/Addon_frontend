...
const WashingFloorSupervisorPage = () => {
  ...
  // Load washing floor orders from API
  const loadOrders = async () => {
    ...
    try {
      ...
      if (response.success) {
        console.log('Washing orders loaded:', response.data.results);
        setOrders(response.data.results);
        setTotalPages(response.data.totalPages);
        setTotalResults(response.data.totalResults);
      } else {
        console.error('Failed to load washing orders:', response.error);
        toast.error('Failed to load washing orders');
      }
    } catch (error: any) {
      console.error('Error loading washing orders:', error);
      toast.error(error.message || 'Failed to load washing orders');
    } finally {
      setIsLoading(false);
    }
  };
