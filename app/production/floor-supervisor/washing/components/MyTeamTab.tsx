  ...
  if (isLoading && members.length === 0) {
    return (
      <div className="p-[10px] flex flex-col items-center justify-center py-20">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-600 mb-4 opacity-50" />
    ...
    <div className="p-[10px]">
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <div className="relative flex-1 min-w-[140px] max-w-[240px]">
          <input
            type="text"
            className="bg-white border border-gray-200 pl-8 pr-3 py-1.5 text-[11px] rounded focus:ring-0 focus:border-cyan-300 w-full placeholder:text-gray-400 font-medium"
                ...
                return (
                  <tr key={row._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-1.5 py-2.5 border border-gray-200">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${
                          row.role === "Supervisor" ? "bg-cyan-50 text-cyan-700 border border-cyan-100" : "bg-gray-50 text-gray-600 border border-gray-200"
                        }`}
                      >
                        {row.role}
                      </span>
                    ...
                    <td className="px-1.5 py-2.5 border border-gray-200">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${
                          row.status === "Active" ? "bg-green-50 text-green-700 border border-green-100" : "bg-gray-50 text-gray-600 border border-gray-200"
                        }`}
                      >
                        {row.status}
                      </span>
            ...
            <div className="flex-1 overflow-y-auto p-[10px]">
              {articleDetailsLoading ? (
                <div className="flex flex-col items-center justify-center py-12">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-600 mb-3 opacity-50" />
