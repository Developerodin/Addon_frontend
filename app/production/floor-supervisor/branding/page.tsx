...
const BrandingFloorSupervisorPage = () => {
  ...
  useEffect(() => {
    ...
    productionService.getArticleProcesses(mongoId).then((res) => {
      ...
      if (res.success && res.data?.processes) {
        const next = resolveNextFloorFromProcesses(res.data.processes, "Branding", "Final Checking");
  ...
  const handleAcceptArticleQuantity = async () => {
    ...
    try {
      ...
      toast.success("Container accepted on Branding.");
      setShowContainerScanDrawer(false);
      setContainerScanned(null);
      setContainerScanBarcode("");
      void loadFloorOrdersCatalog();
  ...
  return (
    <div className="main-content !p-[10px]">
        ...
        <div className="p-[10px]">
          ...
          <div className="flex items-center justify-between border-b border-gray-300 mb-0">
            <div className="flex">
              ...
              <button
                type="button"
                className={`px-3 py-2 text-[11px] font-bold border-b-2 transition-colors ${activeTab === "upcoming" ? "border-amber-600 text-amber-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
              ...
              {!containerScanned ? (
                ...
                <div className="space-y-4">
                  ...
                  {containerScanned.articles.length > 0 ? (
                    ...
                    >
                      {acceptArticleLoading ? "Accepting..." : "Accept container (Branding)"}
                    </button>
                  ) : (
      ...
      {showUpdateContainerModal && selectedOrder && (() => {
        const getTransferTotal = (items: Array<{ transferred?: number }>) => (items ?? []).reduce((s, i) => s + (i.transferred ?? 0), 0);
        const modalArticles = selectedArticleId ? selectedOrder.articles.filter((a) => (a.id ?? a._id) === selectedArticleId) : selectedOrder.articles;
        const articlesWithQty = modalArticles
          .map((a) => {
            const id = a.id ?? a._id;
            if (!id) return null;
            const total = getTransferTotal(updateData[id]?.transferItems ?? []);
            return total > 0 ? { article: a, quantity: total } : null;
          })
          .filter((x): x is { article: Article; quantity: number } => Boolean(x));
        const firstWithQty = articlesWithQty[0];
        const nextFloor = firstWithQty ? (updateContainerNextFloor || "Final Checking") : "";
        return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50" onClick={() => { setShowUpdateContainerModal(false); setUpdateContainerCheckStatus("idle"); setUpdateContainerFetched(null); }} aria-hidden>
          <div className="bg-white rounded-lg shadow-xl border border-gray-200 w-full max-w-sm p-[10px] flex flex-col gap-3" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[11px] text-[#495057]">Scan container for branding completed articles. All articles with transfer qty will be added to the container.</p>
            <div>
              <label className="block text-[11px] font-semibold text-gray-600 mb-1">Container barcode</label>
              <input type="text" placeholder="Scan or enter barcode" value={updateContainerBarcode} onChange={(e) => setUpdateContainerBarcode(e.target.value)} className="w-full border border-gray-200 rounded px-3 py-1.5 text-[11px] focus:ring-0 focus:border-amber-300" />
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (activeItems.some((i) => !i.article)) {
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-600 text-white text-[11px] font-bold rounded hover:bg-amber-700 disabled:opacity-50"
              >
