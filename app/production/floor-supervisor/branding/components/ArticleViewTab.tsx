...
export interface ArticleViewTabProps {
  ...
  isLoading?: boolean;
  onViewOrder: (order: ProductionOrder, article?: { id?: string; _id?: string }) => void;
  onUpdateOrder: (order: ProductionOrder, article?: { id?: string; _id?: string }) => void;
...
/** Branding remaining qty (prefers API remaining, else received − transferred). */
function brandingRemaining(article: { floorQuantities?: { branding?: { received?: number; transferred?: number; remaining?: number } } }): number {
  const branding = article.floorQuantities?.branding;
  if (branding == null) return 0;
  if (typeof branding.remaining === "number") return branding.remaining;
  const received = branding.received ?? 0;
  const transferred = branding.transferred ?? 0;
  return Math.max(0, received - transferred);
}

/** Flattens orders into rows with branding received > 0. */
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => brandingRemaining(r.article) > 0);
  ...
  const handleExportExcel = () => {
    ...
    a.href = url;
    a.download = `branding-article-view-${ts}.csv`;
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No branding articles on this floor</p>
        </div>
      </div>
    );
  }
        ...
        <table className="w-full border-collapse border border-gray-200">
          <thead>
            <tr className="bg-gray-50/30">
              ...
              <th className="px-1.5 py-2.5 text-left text-[11px] font-bold text-[#495057] uppercase tracking-wider border border-gray-200">Branding Type</th>
            ...
            {pagedRows.map(({ article, order }) => {
              ...
              return (
                <tr key={key} className={`hover:bg-gray-50/50 transition-colors group ${isActiveRow ? "ring-2 ring-amber-500 ring-inset bg-amber-50/50" : ""}`}>
                  ...
                  <td className="px-1.5 py-2.5 text-center text-[12px] text-amber-600 font-medium border border-gray-200">{received.toLocaleString()}</td>
                  ...
                  <td className="px-1.5 py-2.5 border border-gray-200 text-[11px] text-gray-700">{article.brandingType ?? "—"}</td>
                  ...
                  <td className="px-1.5 py-2.5 text-right pr-[10px] border border-gray-200">
                    <div className="flex items-center justify-end gap-1 opacity-80 group-hover:opacity-100 flex-wrap">
                      {onAssignClick && (
                        <button
                          type="button"
                          className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold rounded ${isActiveRow ? "bg-amber-600 text-white hover:bg-amber-700 shadow-sm" : "bg-sky-50 text-sky-600 border border-sky-100 hover:bg-sky-100"}`}
            ...
            {Array.from({ length: Math.min(articleTotalPages, 7) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-7 h-7 flex items-center justify-center text-[11px] font-bold rounded ${safeArticlePage === pageNum ? "bg-amber-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-50"}`}
