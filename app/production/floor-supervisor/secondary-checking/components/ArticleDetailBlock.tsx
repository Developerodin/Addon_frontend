...
/**
 * Single article block for "View active article" drawer on Secondary Checking floor.
 * Shows secondaryChecking floor quantities; Article complete button calls DELETE active-article.
 */
export default function ArticleDetailBlock({
  ...
  const machine =
    typeof art.machineId === "object" && art.machineId && "machineCode" in art.machineId
      ...
      : null;
  const sc = (art as any).floorQuantities?.secondaryChecking;
      ...
      <table className="w-full border-collapse text-[10px]">
        <tbody>
          ...
          {sc != null && (
            <>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">Sec Check Rcv</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{sc.received}</td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">Sec Check Rem</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{sc.remaining ?? (sc.received - (sc.transferred ?? 0))}</td>
              </tr>
              {sc.m1Quantity != null && (
                <tr>
                  <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">M1 (Good)</td>
                  <td className="border border-gray-200 px-1.5 py-0.5">{sc.m1Quantity}</td>
                </tr>
              )}
            </>
          )}
