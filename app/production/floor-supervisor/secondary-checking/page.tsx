...
const SecondaryCheckingFloorSupervisorPage = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if (response.success) {
        console.log('Secondary checking orders loaded:', response.data.results);
        setOrders(response.data.results);
        setTotalPages(response.data.totalPages);
        setTotalResults(response.data.totalResults);
      } else {
        console.error('Failed to load secondary checking orders:', response.error);
        toast.error('Failed to load secondary checking orders');
      }
    } catch (error: any) {
      console.error('Error loading secondary checking orders:', error);
      toast.error(error.message || 'Failed to load secondary checking orders');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Filter orders and articles based on received quantity for secondary checking floor only
  const filterOrdersByReceivedQuantity = (orders: ProductionOrder[]): ProductionOrder[] => {
    return orders.map(order => {
      // Filter articles that have received quantity > 0 on secondary checking floor only
      const filteredArticles = order.articles.filter(article => {
        const secondaryCheckingReceived = article.floorQuantities?.secondaryChecking?.received || 0;
        return secondaryCheckingReceived > 0;
      });
  ...
  const handleUpdateSubmit = async () => {
    ...
    const invalidArticles = selectedOrder.articles.filter(article => {
      ...
      const secondaryCheckingFloor = getSecondaryCheckingFloorData(article);
      const received = secondaryCheckingFloor.data?.received || 0;
      const transferred = secondaryCheckingFloor.data?.transferred || 0;
          ...
          <div className="box !bg-transparent border-0 shadow-none">
            <div className="box-header flex justify-between items-center">
              <div className="flex items-center space-x-3">
                <h1 className="box-title text-2xl font-semibold">Secondary Checking Floor Supervisor Dashboard</h1>
                <HelpIcon
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-gray-700">
                          This is the Secondary Checking Floor Supervisor Dashboard where you can view and update production orders that are currently on the Secondary Checking floor.
                        </p>
                      </div>
                    ...
                    <p className="text-2xl font-bold text-white">
                      {orders.reduce((sum, order) => 
                        sum + order.articles.reduce((articleSum, article) => {
                          const secondaryCheckingFloor = getSecondaryCheckingFloorData(article);
                          return articleSum + (secondaryCheckingFloor.data?.m1Quantity || article.m1Quantity || 0);
                        }, 0), 0
                      )}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">
                      {orders.reduce((sum, order) => 
                        sum + order.articles.reduce((articleSum, article) => {
                          const secondaryCheckingFloor = getSecondaryCheckingFloorData(article);
                          return articleSum + (secondaryCheckingFloor.data?.m2Quantity || article.m2Quantity || 0);
                        }, 0), 0
                      )}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">
                      {orders.reduce((sum, order) => 
                        sum + order.articles.reduce((articleSum, article) => {
                          const secondaryCheckingFloor = getSecondaryCheckingFloorData(article);
                          return articleSum + (secondaryCheckingFloor.data?.m3Quantity || article.m3Quantity || 0) + (secondaryCheckingFloor.data?.m4Quantity || article.m4Quantity || 0);
                        }, 0), 0
                      )}
                    </p>
                  </div>
              ...
              {isLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-4">
                      ...
                      : 'No orders currently on Secondary Checking floor'
                    }
                  </p>
                </div>
              ) : (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <div className="space-y-1">
                              ...
                              {order.articles.some(article => article.floorQuantities?.secondaryChecking) && (
                                <div className="text-xs text-blue-600">
                                  Secondary Checking: R:{order.articles.reduce((sum, article) => sum + (article.floorQuantities?.secondaryChecking?.received || 0), 0)} | 
                                  Rem:{order.articles.reduce((sum, article) => sum + (article.floorQuantities?.secondaryChecking?.remaining || 0), 0)}
                                </div>
                              )}
