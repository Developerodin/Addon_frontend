...
type ReBoardingTab = "orders" | "article-view" | "my-team" | "upcoming";
...
const ReBoardingFloorSupervisorPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<ReBoardingTab>("article-view");
  ...
  /** Loads re-boarding floor orders for both tabs; filter + paginate client-side. */
  const loadFloorOrdersCatalog = useCallback(async () => {
    ...
    try {
      ...
      const response = await productionService.getFloorOrders("Re-Boarding", apiFilters);

      if (response.success) {
        setFloorCatalog(response.data.results);
      } else {
        console.error("Failed to load re-boarding floor orders:", response.error);
        toast.error("Failed to load re-boarding orders");
      }
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Failed to load re-boarding orders";
      console.error("Error loading re-boarding floor orders:", error);
      toast.error(msg);
    } finally {
  ...
  useEffect(() => {
    ...
    productionService.getArticleProcesses(mongoId).then((res) => {
      ...
      if (res.success && res.data?.processes) {
        const next = resolveNextFloorFromProcesses(res.data.processes, "Re-Boarding", "Final Checking");
  ...
  const filterOrdersByReceivedQuantity = (orders: ProductionOrder[], showAll: boolean): ProductionOrder[] => {
    return orders.map(order => {
      const filteredArticles = order.articles.filter(article => {
        ...
        const remaining = article.floorQuantities?.reBoarding?.remaining ?? (received - transferred);
  ...
  const qrScan = useProductionArticleQrScan({
    floorApiName: "Re-Boarding",
    floorKey: "reBoarding",
    floorLabel: "Re-Boarding",
  ...
  const getReBoardingFloorData = (article: Article) => ({ floor: "reBoarding" as const, data: article.floorQuantities?.reBoarding });
  ...
  const CURRENT_FLOOR = "Re-Boarding";
  ...
  const handleAcceptArticleQuantity = async () => {
    ...
    try {
      ...
      toast.success("Container accepted on Re-Boarding.");
      setShowContainerScanDrawer(false);
      setContainerScanned(null);
      setContainerScanBarcode("");
      void loadFloorOrdersCatalog();
  ...
  const handleOpenAssignDrawer = useCallback(async () => {
    ...
    try {
      const data = await teamMasterService.list({ workingFloor: "Re-Boarding", limit: 200 });
  ...
  return (
    <div className="main-content !p-[10px]">
      <Seo title="Re-Boarding Supervisor Dashboard"/>
      <div className="bg-white shadow-sm border border-gray-300 overflow-hidden mx-0">
        <div className="p-[10px]">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-sm font-bold text-gray-800">Re-Boarding Floor Supervisor</h1>
              <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm">{filteredOrders.length}</span>
              <HelpIcon
                title="Re-Boarding Floor Supervisor Dashboard"
                content={
                  <div className="space-y-4">
                    <div><h4 className="font-semibold text-lg mb-2">What is this page?</h4><p className="text-gray-700">Re-Boarding Floor Supervisor Dashboard to view and update orders on the Re-Boarding floor.</p></div>
                    <div><h4 className="font-semibold text-lg mb-2">What can you do here?</h4>
                      <ul className="list-disc list-inside space-y-1 text-gray-700">
                        <li><strong>View Orders</strong> — orders with articles on Re-Boarding</li>
        ...
        <div className="min-h-[300px]">
          {activeTab === "my-team" ? (
            ...
            <UpcomingTab floorName="Re-Boarding" />
          ...
          {catalogLoading && floorCatalog.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center"><div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center mb-4"><i className="ri-file-list-line text-xl text-gray-200"></i></div><h3 className="text-xs font-bold text-gray-400 mb-1">NO ORDERS FOUND</h3><p className="text-[10px] text-gray-500">{hasActiveFilters ? 'Try adjusting filters or search' : 'No orders on Re-Boarding floor'}</p></div>
          ) : filteredOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center"><div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center mb-4"><i className="ri-file-list-line text-xl text-gray-200"></i></div><h3 className="text-xs font-bold text-gray-400 mb-1">NO ORDERS WITH REMAINING QTY</h3><p className="text-[10px] text-gray-500">Turn on Show all to include orders with zero remaining on Re-Boarding</p></div>
          ) : (
                ...
                <tbody>
                  {paginatedOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-1.5 py-2.5 border border-gray-300"><div className="text-[12px] font-medium text-gray-600">{order.articles.length} Article{order.articles.length !== 1 ? 's' : ''} · Qty {order.articles.reduce((s, a) => s + (a.plannedQuantity || 0), 0).toLocaleString()}</div>{order.articles.some(a => (a as any).floorQuantities?.reBoarding) && <div className="text-[10px] text-green-600 mt-0.5">R:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.reBoarding?.received || 0), 0)} Trf:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.reBoarding?.transferred || 0), 0)} Rem:{order.articles.reduce((s, a) => s + ((a as any).floorQuantities?.reBoarding?.remaining ?? 0), 0)}</div>}</td>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-4xl bg-white shadow-xl z-50 flex flex-col overflow-hidden animate-slide-in-right border-l-2 border-gray-300">
            <div className="flex items-center justify-between gap-3 px-3 py-2 border-b-2 border-gray-300 bg-gray-50 flex-shrink-0">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button type="button" onClick={closeUpdateModal} className="text-gray-500 hover:text-gray-800 p-1 rounded border-2 border-gray-300 hover:bg-gray-100" aria-label="Close drawer"><i className="ri-close-line text-lg"></i></button>
              </div>
            </div>
        ...
        <>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl border-2 border-gray-300 max-w-md w-full p-4">
              ...
              <p className="text-[11px] text-gray-600 mb-3">Scan container for re-boarding completed articles. All articles with completed qty will be added.</p>
      ...
      {showAssignDrawer && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-xl z-50 flex flex-col overflow-hidden animate-slide-in-right">
            <div className="flex justify-between items-center p-[10px] border-b border-gray-200"><h3 className="text-sm font-bold text-gray-800">Assign to team member (Re-Boarding)</h3><button type="button" onClick={handleCloseAssignDrawer} className="text-gray-500 hover:text-gray-700 p-1"><i className="ri-close-line text-lg" /></button></div>
                  ...
                  <table className="min-w-full text-xs border-collapse">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {modalArticles.map((article, idx) => {
                        const plannedQty = article.plannedQuantity || 0;
                        const receivedQty = article.floorQuantities?.reBoarding?.received || 0;
                        const completedQty = article.floorQuantities?.reBoarding?.completed || 0;
                        const transferredQty = article.floorQuantities?.reBoarding?.transferred || 0;
                        const remainingQty = article.floorQuantities?.reBoarding?.remaining || 0;
                          ...
                          <tr key={article.id || article._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-2 py-1.5 border-r border-gray-300 align-top min-w-[120px]">
                              <ReceivedQuantityDisplay
                                ...
                                repairReceived={article.floorQuantities?.reBoarding?.repairReceived}
                                repairFromFloor={article.floorQuantities?.reBoarding?.repairFromFloor}
                              />
                            </td>
...
export default ReBoardingFloorSupervisorPage;
