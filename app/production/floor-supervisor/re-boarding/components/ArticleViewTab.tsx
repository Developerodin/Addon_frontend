...
/** Re-Boarding remaining qty (prefers API remaining, else received − transferred). */
function reBoardingRemaining(article: Article): number {
  const boarding = article.floorQuantities?.reBoarding;
  if (boarding == null) return 0;
  if (typeof boarding.remaining === "number") return boarding.remaining;
  const received = boarding.received ?? 0;
  const transferred = boarding.transferred ?? 0;
  return Math.max(0, received - transferred);
}

/** Flattens orders into rows with re-boarding received > 0. */
...
export default function ArticleViewTab({
  ...
  const visibilityFilteredRows = useMemo(() => {
    ...
    return articleRows.filter((r) => reBoardingRemaining(r.article) > 0);
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredRows.map(({ article, order }) => {
      ...
      const board = article.floorQuantities?.reBoarding;
      const received = board?.received ?? 0;
      const done = board?.completed ?? article.completedQuantity ?? 0;
    ...
    a.href = url;
    a.download = `re-boarding-article-view-${ts}.csv`;
    ...
    return (
      <div className="p-[10px]">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[10px] text-gray-500">No re-boarding articles on this floor</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-[10px]">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <div className="relative flex-1 min-w-[140px] max-w-[240px]">
          <input
            type="text"
            className="bg-white border border-gray-200 pl-8 pr-3 py-1.5 text-[11px] rounded focus:ring-0 focus:border-green-300 w-full placeholder:text-gray-400 font-medium"
        ...
        <table className="w-full border-collapse border border-gray-200">
          ...
          <tbody>
            {pagedRows.map(({ article, order }) => {
              ...
              const board = article.floorQuantities?.reBoarding;
              const received = board?.received ?? 0;
              const completed = board?.completed ?? article.completedQuantity ?? 0;
              const transferred = board?.transferred ?? 0;
              const remaining = board?.remaining ?? Math.max(0, received - transferred);
              ...
              return (
                <tr
                  key={key}
                  className={`hover:bg-gray-50/50 transition-colors group ${isActiveRow ? "ring-2 ring-green-500 ring-inset bg-green-50/50" : ""}`}
                >
                  ...
                  <td className="px-1.5 py-2.5 text-right pr-[10px] border border-gray-200">
                    <div className="flex items-center justify-end gap-1 opacity-80 group-hover:opacity-100 flex-wrap">
                      {onAssignClick && (
                        <button
                          type="button"
                          className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold rounded ${isActiveRow ? "bg-green-600 text-white hover:bg-green-700 shadow-sm" : "bg-green-50 text-green-600 border border-green-100 hover:bg-green-100"}`}
      ...
      {filteredRows.length === 0 && !articleSearch.trim() && orders.length > 0 && (
        <div className="py-8 text-center text-[11px] text-gray-500">
          {showAllArticles
            ? "No re-boarding lines to display."
            ...
            {Array.from({ length: Math.min(articleTotalPages, 7) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-7 h-7 flex items-center justify-center text-[11px] font-bold rounded ${safeArticlePage === pageNum ? "bg-green-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-50"}`}
