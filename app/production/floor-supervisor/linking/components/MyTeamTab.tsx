...
const FLOOR_LINKING = "Linking" as const;

/**
 * Shows all team members whose workingFloor is Linking (read-only).
 * Used on the Linking Floor Supervisor page under "My Team" tab.
 */
    ...
    <div className="p-[10px]">
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <span className="text-[11px] font-medium text-[#495057]">
          {totalResults} member{totalResults !== 1 ? "s" : ""} on Linking floor
        </span>
      </div>
            ...
            {members.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-[11px] text-gray-500">
                  No team members found for Linking floor.
                </td>
              </tr>
            ) : (
              members.map((row) => {
                    ...
                    <td className="px-1.5 py-2.5 border border-gray-200">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${
                          row.role === "Supervisor" ? "bg-blue-50 text-blue-700 border border-blue-100" : "bg-gray-50 text-gray-600 border border-gray-200"
                        }`}
                      >
                        {row.role}
                      </span>
      ...
      {/* View active articles – drawer with article details (Excel-like) */}
      {viewActiveMember && (
        <>
