...
/**
 * Single article block for "View active article" drawer – Excel-like small font, order + machine refs.
 * Optional: Article complete button calls DELETE active-article and removes from list.
 */
export default function ArticleDetailBlock({
  ...
  /** Use MongoDB _id for team-masters active-article DELETE; backend expects _id not articleNumber */
  ...
  const machine =
    typeof art.machineId === "object" && art.machineId && "machineCode" in art.machineId
      ...
      : null;
  const link = art.floorQuantities?.linking;
      ...
      <table className="w-full border-collapse text-[10px]">
        <tbody>
          ...
          {link != null && (
            <>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">Linking Rcv</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{link.received}</td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-1.5 py-0.5 font-medium text-gray-600 bg-gray-100">Linking Rem</td>
                <td className="border border-gray-200 px-1.5 py-0.5">{link.remaining ?? link.received - (link.transferred ?? 0)}</td>
              </tr>
            </>
          )}
          {order && (
