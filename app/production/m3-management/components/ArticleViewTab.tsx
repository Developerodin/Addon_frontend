...
import type { M3ArticleRow } from "@/shared/services/productionService";
...
/**
 * Article-wise M3 table with combined totals (all floors summed).
 */
export default function ArticleViewTab({
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        (r.orderNote ?? "").toLowerCase().includes(q)
    );
  }, [rows, search]);
  ...
  /** Export filtered M3 article rows as CSV. */
  const handleExportExcel = () => {
    if (filtered.length === 0) {
      toast.error("No M3 articles to export");
      return;
    }

    const header = ["Order", "Order Note", "Article", "Combined M3", "Outward", "Available"];
    ...
    downloadCsv(datedExportFilename("m3-articles"), [header, ...lines]);
    ...
    <div>
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <DownloadExcelButton
          ...
          ariaLabel="Export filtered M3 articles to Excel"
          className="ml-auto"
        />
      </div>
      ...
      <div className="border border-gray-300 rounded overflow-hidden overflow-x-auto">
        <table className="w-full border-collapse border border-gray-300 text-[10px] table-fixed min-w-[640px]">
          <thead className="bg-gray-100">
            <tr>
              <th className="border border-gray-300 px-1 py-1 text-left w-[140px]">Order</th>
              <th className="border border-gray-300 px-1 py-1 text-left w-[100px]">Article</th>
              ...
              paged.map((row) => {
                ...
                return (
                  <tr key={row._id ?? row.id} className="hover:bg-gray-50/50">
                    <td className="border border-gray-300 px-1 py-1 min-w-0">
                    ...
                    <td className="border border-gray-300 px-1 py-1 font-semibold">{row.articleNumber}</td>
                    ...
                    <td className="border border-gray-300 px-1 py-1 text-center">
                      <div className="flex justify-center gap-1">
                        ...
                        <button
                          ...
                          aria-label={`View M3 detail for ${row.articleNumber}`}
                        >
                          View
                        </button>
                        <button
                          ...
                          aria-label={`Mark outward for ${row.articleNumber}`}
                        >
                          Outward
                        </button>
                      </div>
      ...
      <p className="text-[10px] text-gray-500 mt-2">
        Combined M3 = sum across Checking, Secondary Checking, and Final Checking only. Use Orders tab for per-floor breakdown.
      </p>
      {productImageModal}
    </div>
  );
