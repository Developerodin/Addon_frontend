...
const AddStyleCodePairPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await styleCodePairsService.create({
      ...
      toast.success("Style code pair created")
      router.push("/catalog/style-code-pairs")
    } catch (error) {
      console.error("Create failed", error)
      toast.error("Failed to create style code pair")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="main-content !p-[10px]">
      <Seo title="Add Style Code Pair" />
      <Toaster position="top-right" />

      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-[3px] h-5 bg-purple-600 rounded-full" />
          <h1 className="text-sm font-bold text-gray-800">Add Style Code Pair</h1>
        </div>
        <Link
...
export default AddStyleCodePairPage
